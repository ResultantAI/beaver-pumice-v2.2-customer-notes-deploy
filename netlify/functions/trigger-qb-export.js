...
exports.handler = async (event, context) => {
  ...
  let options = {};
  try { options = JSON.parse(event.body || '{}'); } catch (e) { options = {}; }
  
  const TO_EMAIL = options.testEmail || 'accounting@beaverpumice.com';
  const CC_EMAIL = options.skipCC ? null : 'lucas@beaverpumice.com';
  ...
  if (!RESEND_API_KEY && !dryRun) {
    return { statusCode: 500, headers, body: JSON.stringify({ error: 'Missing Resend API key' }) };
  }

  console.log('=== TRIGGER QB EXPORT v74 STARTED ===');
  ...
  try {
    ...
    if (pendingTickets.length === 0) {
      return {
        statusCode: 200,
        headers,
        body: JSON.stringify({ success: true, message: 'No pending tickets to export', ticketsFound: 0 })
      };
    }
    ...
    // v74: Pass products to generateIIF
    ...
    if (dryRun) {
      return {
        ...
        body: JSON.stringify({
          success: true,
          dryRun: true,
          ticketsFound: pendingTickets.length,
          invoicesGenerated: invoiceCount,
          preview: iifContent.substring(0, 2000),
          tickets: pendingTickets.map(t => ({ number: t.number, customer: t.customerName }))
        })
      };
    }
    ...
    if (CC_EMAIL) emailOptions.cc = CC_EMAIL;
    ...
    return {
      ...
      body: JSON.stringify({
        success: true,
        ticketsExported: pendingTickets.length,
        invoicesGenerated: invoiceCount,
...
async function fetchPendingTickets(token, baseId) {
  ...
  return allRecords.map(record => {
    ...
    // v74: Read lookup fields from ticket (these come from Customers table via lookup)
    ...
    return {
      ...
      // v74: Airtable's pre-calculated values
...
function getQBItemCode(productName, productId, products, ticketQBCode) {
  ...
  return productName;  // Will cause QB error - alerts user
}
